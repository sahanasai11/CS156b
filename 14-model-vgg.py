...
combined = copy.deepcopy(preds_no_finding)

for i in range(len(tensor_test)):
    img_id = test_img_id[i]
    
    preds_enlarged_cardio.append(preds_enlarged_cardio[i][0])
    preds_cardio.append(preds_cardio[i][0])
    preds_lung_opacity.append(preds_lung_opacity[i][0])
    preds_lung_lesion.append(preds_lung_lesion[i][0])
    preds_edema.append(preds_edema[i][0])
    preds_consolidation.append(preds_consolidation[i][0])
    preds_pneumonia.append(preds_pneumonia[i][0])
    preds_atelectasis.append(preds_atelectasis[i][0])
    preds_pneumothorax.append(preds_pneumothorax[i][0])
    preds_pleural_effusion.append(preds_pleural_effusion[i][0])
    preds_pleural_other.append(preds_pleural_other[i][0])
    preds_fracture.append(preds_fracture[i][0])
    preds_support_devices.append(preds_support_devices[i][0])
